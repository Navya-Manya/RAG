...
st.set_page_config(page_title="Document Reader Engine")
st.title("Document Reader Engine with RAG & Pinecone")


def load_documents():
    """
    Loads documents from a directory using the DirectoryLoader.
...
def embeddings_on_local_vectordb(texts):
    """
    Generates embeddings and sets up a local vector database.

    Args:
    texts (list): List of text chunks.

    Returns:
    retriever: Retriever object for the local vector database.
    """
...
def embeddings_on_pinecone(texts):
    """
    Generates embeddings and sets up a vector database on Pinecone.

    Args:
    texts (list): List of text chunks.

    Returns:
    retriever: Retriever object for the Pinecone vector database.
    """
...
def query_llm(retriever, query):
    """
    Queries the language model using a retriever.

    Args:
    retriever: Retriever object for retrieving relevant documents.
...
def input_fields():
    """
    Defines the input fields for providing API keys and environment details.
    """
    #
    with st.sidebar:
        #
...
def process_documents():
    """
    Process the uploaded documents to generate retrievers based on the selected options.
    """
...
def launch():
    """
    Launch the Streamlit app, including input fields, document submission button,
    display of chat messages, and processing of user queries.
    """
    #
    input_fields()
    #
